package leetcode._2ndtime.no188;

import java.util.Arrays;

public class Solution {
    int[] dpSold = new int[10000];
    int[] dpHold = new int[10000];
    int[] dpSoldPrev = new int[10000];
    int[] dpHoldPrev = new int[10000];
...
//        int[] dpSold = new int[k+1];
//        int[] dpHold = new int[k+1];
//        int[] dpSoldPrev = new int[k+1];
//        int[] dpHoldPrev = new int[k+1];
        Arrays.fill(dpHold, Integer.MIN_VALUE);
        Arrays.fill(dpHoldPrev, Integer.MIN_VALUE);
        Arrays.fill(dpSold, 0);
        Arrays.fill(dpSoldPrev, 0);

        int max = 0;
        for (int i = 0; i < prices.length; i++) {
            int kTop = Math.min(k, (i+1) / 2);
            for (int kk = 0; kk <= k; kk++) {
                if (kk == 0) dpSold[kk] = 0;
                else dpSold[kk] = Math.max(dpSoldPrev[kk], dpHoldPrev[kk-1] + prices[i]);

                dpHold[kk] = Math.max(dpHoldPrev[kk], dpSoldPrev[kk] - prices[i]);

                max = Math.max(max, dpSold[kk]);
            }
